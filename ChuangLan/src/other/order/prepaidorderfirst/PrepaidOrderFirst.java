 ...
 * @author gongqiang
 */
public class PrepaidOrderFirst implements ScriptTrigger {
    private static final Logger logger = LoggerFactory.getLogger();
    private static final Long ENTITY_TYPE = 735708796617355L;

    @Override
    public ScriptTriggerResult execute(ScriptTriggerParam scriptTriggerParam) throws ScriptBusinessException {
        logger.info("预付订单-是否首冲状态回写-开始");
        try {
            ...
            if (ENTITY_TYPE.equals(entityType)) {
                //客户ID
                Long accountId = model.getLong("accountId");
                //今天零点
                ...
                if (firstOrderFlag) {
                    ...
                    logger.info("客户ID:"+accountId+"首充 订单");
                } else {
                    model.setAttribute("customItem188__c", 2);
                    logger.info("客户ID:"+accountId+"非首充 订单");
                }
            }
        } catch (Exception ex) {
            logger.error("预付订单-是否首冲状态回写 错误Exception：" + ex.toString());
        }
