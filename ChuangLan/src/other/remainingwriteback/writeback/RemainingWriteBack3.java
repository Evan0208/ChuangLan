 ...
 * @author gongqiang
 */
public class RemainingWriteBack3  implements ScriptTrigger {
    ...
    public ScriptTriggerResult execute(ScriptTriggerParam scriptTriggerParam) throws ScriptBusinessException {
        logger.info("月结订单(万数闪验)-剩余数量回写-开始");
        try {
            ...
            logger.info("月结订单(万数闪验) 保存，查询自助通账号" + apiAccount + " 的余额：" + balance);
            if (balance != null) {
                //剩余数量
                model.setAttribute("customItem5__c", balance);
            } else {
                logger.error("月结订单(万数闪验)保存，查询自助通账号" + apiAccount + " 的余额失败。");
            }

        } catch (Exception ex) {
            logger.error("月结订单(万数闪验)-剩余数量回写,错误Exception：" + ex.getMessage().toString());
        }
